...
**🗺️ 地图已收藏功能**
- 新增"地图已收藏"复选框，勾选后跳过分享码输入，改为 PageDown × 7 直接导航
- **⚠️ 注意：用来刷点的图必须是切到收藏时的第一个！**
- 蓝图的 VEI 赛事信息检测对两条路径均生效，保证地图加载完成后再选车
- 设置保存到 config.json，支持方案独立配置
...
**🔧 错误处理改进**
- 全项目 34 处 `except Exception: pass` 静默吞错，关键路径 10 处改为带日志输出（ERROR/WARN/DEBUG）
- 涉及配置读写、模板匹配、截图、坐标计算、恢复逻辑等核心环节，出问题不再黑箱
- UI 装饰、剪贴板、cleanup 等非关键路径保留 pass

**🚗 超级抽奖逻辑增强**
- 翻页搜索从 3 页扩展到 5 页，覆盖更多车辆
- 新增品牌重试机制：5 页未找到 -> 重新进入选品牌 -> 再找 5 页 -> 仍未找到才退出
- 智能记忆页码在重试时自动重置

**🗑️ 删除车辆逻辑增强**
- 翻页搜索从 5 页扩展到 10 页，覆盖更多消耗品车辆
